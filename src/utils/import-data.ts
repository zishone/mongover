...
export async function importData(collection: Collection, dataSpec: DataSpec, dataPath: string): Promise<void> {
  try {
    logger.debug('Importing Data: %s', dataPath);
    logger.cli('------- Importing Data:             %s', dataPath);
    ...
    logger.error('Error importing Data: %s', collection.namespace);
    logger.cli('------- Error importing Data:       %s', dataPath);
    throw error;
  }
}
